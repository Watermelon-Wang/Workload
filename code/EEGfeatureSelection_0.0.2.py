...
def getDataFromSubject(data, subjectIndex):
    ...
    highLabel = np.zeros(sampleSize)
    lowLabel = np.ones(sampleSize)
...
def testClassifer(trainData, testData, trainLabel, testLabel):
    ...
    classifiersName = ['NB','KNN','LR','RF','DT','GBDT','NuSVC','LSVC','SVC','XGB']
...
def testClassiferPipeline(dataFile):
    ...
    for subjectIndex in range(25):
        ...
        EEG = getDataFromSubject(data, subjectIndex)
    ...
    with open(r'C:\Users\Summer\Desktop\123.csv', 'w+') as file:
        for i in resultAll:
            for j in i:
                file.write(str(j) + ',')
            file.write('\n')
...
############################# Main Body #############################
    ...
    dataFile = "E:\硕士课题\眼动程序V2\AllOneChannelDiffFreqV003"
    ## 读取Matlab提取的特征（.mat）
    data = scipy.io.loadmat(dataFile)
